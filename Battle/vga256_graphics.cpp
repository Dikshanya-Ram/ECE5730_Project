...
//DLR codes - from Bruce land files
// get the color of a pixel for VGA 256-color mode
short readPixel(short x, short y) {
  // Which pixel is it?
  int pixel = (320 * y) + x;  // Calculate pixel index
  short color;

  // In VGA 256-color mode, each pixel is stored in a separate byte
  color = vga_data_array[pixel];  // Directly access the pixel data
